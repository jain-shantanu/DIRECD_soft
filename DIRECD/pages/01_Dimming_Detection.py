...
st.set_page_config(page_title='Dimming Detection',
                   page_icon=logo_folder_short)
st.markdown(f"You are running the kernal on the directory {current_dir}. If you are using pre-downloaded files - see manual for folder requirements.")
...
if st.session_state.folder_path is not False:
    ...
    default_time_range=150
    try:
        data_event = pd.read_csv(os.path.join(st.session_state.folder_path, 'list_events.csv'))
        st.write(os.path.join(st.session_state.folder_path, 'list_events.csv'))
    ...
    with form.expander("Save Options"):
        ...
        '''
        download_fits = st.checkbox('Download/Overwrite Raw fits (from VSO)',label_visibility="visible",help=help_txt)
        help_txt = '''
    ...
    if submit:
        ...
        if should_download:
            ...
            if tot_files > tot_files_2 or download_fits:
                st.write(f'Data will download in {save_path_fits}. To stop data download, manually add files and rerun')
        ...
        if should_calibrate:
            ...
            save_path_fits = os.path.join(selected_folder_path, 'Events', safe_event, 'fits',str(wavelength),str(cadence))
            diff_rot_folder = os.path.join(selected_folder_path, 'Events', safe_event, 'Calibrated',str(wavelength),str(cadence))
            os.makedirs(diff_rot_folder, exist_ok=True)
            log_diff_folder = os.path.join(selected_folder_path, 'Events', safe_event, 'LBR',str(wavelength),str(cadence))
            ...
            tot_files_cal = len(sorted(os.listdir(diff_rot_folder)))
            tot_files_2 = number-1  # Exclude basemap image
            if tot_files > tot_files_cal or overwrite_files_calibration:
    ...
    # Exclude basemap image
        ...
        ## Creating Base Diff and LBR
            ...
            tot_files_lbr = len(sorted(os.listdir(log_diff_folder)))
            ...
            if tot_files > tot_files_lbr or overwrite_files_lbr:
        ...
        """)
            ...
            save_path_detection = os.path.join(selected_folder_path, 'Events', safe_event, 'Detection_figures',str(wavelength),str(cadence))
            ...
            if save_plots_checkbox_all:
                ...
                with st.spinner(f"Generating Detection Video (Threshold: {lbr_thresh})...",show_time=True):
                    detection_video_path = os.path.join(selected_folder_path, 'Events', safe_event, 'Detection_video',str(wavelength),str(cadence))
